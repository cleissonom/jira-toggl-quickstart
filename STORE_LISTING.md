...
5. Click both a retained Jira-linked appointment and an appointment named like `[ECP-3217] Example`, then confirm each underlined name opens its exact Jira issue in a new tab. Confirm a name without a valid Jira key remains plain text. Then use **Play** on a previous row and confirm any current timer stops before the selected description starts with the saved workspace, optional project, and Billable default.
