...
- Side-panel appointment links are built only from the validated HTTPS Jira origin and a strict retained or description-derived Jira issue key, open in a separate tab with opener access disabled, and are omitted for invalid appointments. Description-derived keys are navigation-only and never become Work Log associations.
