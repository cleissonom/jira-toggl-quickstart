...
- Automatic Work Log creation or confirmation from the extension side panel.
...
- A live **Today's appointments** list with per-appointment totals, links from retained Jira work or Jira-formatted descriptions to the issue, and a **Play** action that safely switches from the current timer.
...
- Configurable floating Jira controls in the top-left, top-right, bottom-left, or bottom-right corner.
...
- Floating Jira control position.
...
The adjacent **Copy** action prepares a Markdown document containing the selected issue key, summary, and description. Jira Cloud Atlassian Document Format is converted locally; plain-string descriptions are also supported. The clipboard is written only after the user clicks **Copy**. Settings can place this action group in the top-left, top-right, bottom-left, or bottom-right corner.
...
The same response powers **Today's appointments**. Known Jira sessions are grouped by their retained issue association; other named entries are grouped by normalized description. Each row shows its total for the local day. An underlined appointment name opens the exact Jira issue in a new tab when the row has either a retained association or a valid Jira key in its description; names without a valid key remain plain text. **Play** validates the source entry, stops any different current timer, and starts the selected description with the currently configured workspace, optional project, and Billable default. The running row and its total update locally. The side panel uses one browser-managed vertical scroll surface for long lists and narrow layouts.

When Play stops a Jira-linked timer, the normal automatic, confirmation, or retry Work Log path runs before the selected appointment starts. Replaying an appointment with a retained Jira association links the new Toggl entry to the same Jira issue, so its later stop follows the same Work Log behavior. A key inferred from a description is navigation-only: manual entries remain Toggl-only even when their names link to a Jira issue.
...
- Replaying a today's appointment with a retained Jira association links the new timer to the same issue.
- Stopping it from Jira, from the side panel, or through automatic timer switching can create the Work Log.
- If that linked timer is stopped directly in Toggl, opening or refocusing the extension side panel reconciles it and attempts the Work Log synchronization.
- Timers entered manually in the side panel are not guessed or matched to Jira issues.

Available behavior:

- **Automatic** — attempt the Jira Work Log as soon as the timer stops.
- **Ask before syncing** — keep the completed entry pending until **Sync pending Work Logs** is clicked in the side panel.
...
Jira page content script
    ...
    ├── renders the positioned timer and copy actions
    ├── copies a worker-prepared Jira document after an explicit click
...
Manifest V3 service worker
    ...
    ├── aggregates daily, weekly, and per-appointment Toggl time for the side panel
    ├── validates replay sources before switching timers
    ├── reads Jira summary, description, and time-tracking fields for the active issue
    ├── converts Jira ADF to Markdown for the authorized Jira copy action
...
- `storage` — saves the token, workspace and optional project configuration, floating-button position, preferences, bounded Jira-linked timer associations, and pending Work Log retry records in the current Chrome profile.
...
This command checks JavaScript syntax, validates `manifest.json`, and runs mocked service-worker, content-script, side-panel, settings, and UI contract tests. Coverage includes optional and automatic project selection, daily/weekly/per-appointment totals, replay validation and switching, floating positions, running-entry semantics, toolbar icon states, Jira progress, ADF-to-Markdown conversion, clipboard states, Work Log creation, retries, duplicate prevention, external-stop reconciliation, and icon dimensions.
...
content.js          Jira issue detection, positioned timer/copy actions, and metadata lookup
options.*           Setup page, Work Log preferences, position, and advanced options
popup.*             Chrome side-panel totals, replayable appointments, timers, Jira progress, and Work Log retry UI
